 ...
 */
import { normalizeDesc } from "../normalize-desc.js";
...
export function computeInvoice(inv, costByKey = {}, cfg = PNL_CONFIG) {
  ...
  const aligns = lines.filter((l) => lineCategory(l) === "alignment" && Number(l.total) !== 0);  // ignore $0 "needs alignment" notes

  // A refund has a negative total — reverse everything (qty, cost, commission) so it
  // nets against the original sale in the day's totals.
  const refund = Number(inv.total) < 0 || Number(inv.subtotal) < 0;
  const sign = refund ? -1 : 1;

  const tireQty = sign * tires.reduce((s, l) => s + l.qty, 0);
  const tpmsQty = sign * tpms.reduce((s, l) => s + l.qty, 0);
  const oilQty = sign * oils.reduce((s, l) => s + l.qty, 0);
  const alignCount = sign * aligns.reduce((s, l) => s + (l.qty || 1), 0);
  ...
  const costOf = (l, category) => {
    ...
    return Number(c.unit_cost) * l.qty * sign;  // negative on refunds (inventory comes back)
  };
  ...
  // Refunds claw back commission (allow negative); normal sales floor at $0.
  const rawComm = Math.trunc(adjustedProfit * cfg.commissionRate);
  const commission = (eligible && !unmatched) ? (refund ? rawComm : Math.max(0, rawComm)) : 0;

  // Per-invoice flags
  const netPerTire = tireQty ? totalProfit / tireQty : null;
  const flags = [];
  if (refund) flags.push("refund");                                   // ↩️ reverses a sale
