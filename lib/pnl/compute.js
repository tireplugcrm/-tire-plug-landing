...
export function computeInvoice(inv, costByKey = {}, cfg = PNL_CONFIG) {
  ...
  let productCost = 0, etcCost = 0;
  const tireDetail = tires.map((l) => {
    const cost = costOf(l, "tire");
    if (cost != null) productCost += cost;
  ...
  for (const l of tpms) { const c = costOf(l, "tpms"); if (c != null) etcCost += c; }  // TPMS sensors
  for (const l of oils) { const c = costOf(l, "oil"); if (c != null) etcCost += c; }   // oil change / filter / motor oil
  const hasAllCosts = missing.length === 0;

  const totalCost = r2(productCost + etcCost);        // COGS = tires + TPMS + oil
  ...
  return {
    ...
    description: [...tires, ...oils, ...tpms].map((l) => l.description).join(" · ") || summarize(lines),
