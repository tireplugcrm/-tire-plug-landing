...
const money = (s) => Number(String(s == null ? "" : s).replace(/[^0-9.-]/g, "")) || 0;  // keep "-" for refunds
...
// ---------------------------------------------------------------- Sales Journal
const JOURNAL_FOOTER = /^(TOTALS|INVOICE\/SALES|TOTAL|Cash:|Zelle:|Card:|LOCAL ROA|CORPORATE ROA|SALES (LIST|JOURNAL)|Inv #|Inv By|DATE:|THE TIRE PLUG|2220 |\(562\)|Los Angeles)/i;
  ...
  for (const line of lines) {
    ...
    if (m) {
      ...
      last = {
        ...
        payment_method: null, _gotPm: false,
      };
      invoices.push(last);
      continue;
    }
    const pm = line.match(/^PM\s+([A-Za-z ]+):\s*\$\s*([\d,]+\.\d{2})/);
    if (pm && last) { last.payment_method = pm[1].trim(); last._gotPm = true; continue; }
    // A long customer name can wrap onto its own line (e.g. "...Luxury" / "Transporting")
    // before the payment line — stitch it back onto the customer.
    if (last && !last._gotPm && /[A-Za-z]/.test(line) && !/\$/.test(line) && !JOURNAL_FOOTER.test(line)) {
      last.customer = `${last.customer} ${line}`.trim();
    }
  }
  invoices.forEach((i) => delete i._gotPm);
  return invoices;
}

// ---------------------------------------------------------------- Sales Detail
const ITEM_RE = /^(Tires|Services|Parts)\s+(.+?)\s+\$\s*(-?[\d,]+\.\d{2})\s+\$\s*(-?[\d,]+\.\d{2})\s+\$\s*(-?[\d,]+\.\d{2})\s+\$\s*(-?[\d,]+\.\d{2})\s+([\d,]+\.\d{2})\s+\$\s*(-?[\d,]+\.\d{2})$/;
// TireBase inserts $0 location-marker rows ("Parts Olympic" / "Parts Manchester") — skip them.
const LOCATION_MARKER = /^(olympic|manchester)$/i;
...
export function parseSalesDetail(lines) {
  ...
  for (const raw of lines) {
    ...
    if (it) {
      ...
      if (LOCATION_MARKER.test(desc)) { lastItem = null; continue; }   // skip $0 location rows
...
export function matchDay(journalInvoices, detailInvoices) {
  ...
  for (const j of journalInvoices) {
    const candidates = (byName[normName(j.customer)] || []).filter((d) => !d._used);
    let det = null;
    ...
    out.push({ ...j, detail_no: det ? det.detail_no : null, lines: det ? det.lines : [], matched: !!det });
  }
  // Fallback: stitch any still-unmatched invoice to a leftover detail with the SAME total,
  // but only when that total is unique among what's left (avoids wrong matches).
  for (const o of out) {
    if (o.matched) continue;
    const sameTotal = detailInvoices.filter((d) => !d._used && Math.abs(d.total - o.total) < 0.01);
    if (sameTotal.length === 1) {
      const d = sameTotal[0]; d._used = true;
      o.detail_no = d.detail_no; o.lines = d.lines; o.matched = true;
    }
  }
  return out;
}
