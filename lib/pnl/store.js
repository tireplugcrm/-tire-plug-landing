...
import { computeDay, computeWeek, lineCategory } from "./compute.js";
...
// Every costable item (tire/TPMS/oil) used in a week, with its current cost — powers
// the "Edit costs" panel so the owner can correct a cost anytime.
export async function getWeekCostItems(start) {
  const costByKey = await loadCostBook();
  const { data } = await supabaseAdmin.from("pnl_invoices").select("lines").in("biz_date", weekDates(start));
  const map = {};
  for (const row of data || []) for (const l of row.lines || []) {
    const category = lineCategory(l);
    if (!["tire", "tpms", "oil"].includes(category)) continue;
    const key = normalizeDesc(l.description);
    if (!map[key]) map[key] = { key, description: l.description, category, unit_cost: costByKey[key] != null ? Number(costByKey[key].unit_cost) : null, qty: 0 };
    map[key].qty += Number(l.qty) || 0;
  }
  const order = { tire: 0, tpms: 1, oil: 2 };
  return Object.values(map).sort((a, b) => order[a.category] - order[b.category] || a.description.localeCompare(b.description));
}
