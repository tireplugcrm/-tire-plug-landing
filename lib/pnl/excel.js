...
const FLAG_EMOJI = { negative: "🟥", low: "🟨", high: "🟩", pickup: "📦", missing: "🟧" };

function rowFill(flags) {
  for (const f of ["missing", "negative", "low", "high", "pickup"]) if (flags.includes(f)) return FILL[f];
  return null;
}
...
function addDaySheet(wb, day) {
  ...
  ws.columns = [
    ...
    { header: "PRODUCT COST", key: "pcost", width: 13 },
    { header: "ETC COST", key: "ecost", width: 11 },
  ...
  for (const r of day.rows) {
    const row = ws.addRow({
      ...
      pcost: r.productCost, ecost: r.etcCost, tcost: r.totalCost, retail: r.retail, tax: r.tax,
      profit: r.totalProfit, comm: r.commission || "",
      flags: r.flags.map((f) => FLAG_EMOJI[f]).join(" "),
    });
    ["pcost", "ecost", "tcost", "retail", "tax", "profit", "comm"].forEach((k) => { row.getCell(k).numFmt = MONEY; });
  ...
  const t = ws.addRow({
    ...
    tcost: day.cogs, retail: day.revenue, tax: day.tax, profit: day.grossProfit, comm: day.commissions,
  });
  t.font = { bold: true };
  ["tcost", "retail", "tax", "profit", "comm"].forEach((k) => { t.getCell(k).numFmt = MONEY; });
