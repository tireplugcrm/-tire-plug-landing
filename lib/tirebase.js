...
function categorize(desc, partNumber) {
  ...
  if (d.includes("balance") || d.includes("rotation")) return "other";
  // GEN0001 = TireBase's generic tire SKU. Also catch dot-separated (215.55R17)
  // and LT/truck (35X12.50R20) size formats the old regex missed.
  if ((partNumber || "").toUpperCase() === "GEN0001") return "tire";
  if (/\d{3}\s*[\/.]\s*\d{2}/.test(d) || /\d{2}x\d{2}/.test(d) || /\b\d{7}\b/.test(d) || d.includes("tire"))
    return "tire";
  return "other";
}
...
export async function fetchDayScoreboard({ date, store_id = 1 } = {}) {
  ...
  rows.forEach((row, i) => {
    ...
    const cat = categorize(row.description, row.part_number);
