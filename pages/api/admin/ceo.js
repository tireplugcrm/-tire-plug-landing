/**
 * CEO Agent — reads the CRM leads, shop-floor jobs, and reactivation opportunity
 * and writes a plain-English daily briefing with specific recommendations. Password/Google gated.
 */
...
export default async function handler(req, res) {
  ...
  // 2) Ops snapshot — jobs on the floor + reactivation opportunity
...
- 2-3 punchy sentences summarizing where things stand (lead pipeline, what's strong, what's lagging).
- Then a blank line, then 2-4 bullet recommendations starting with "•", each a specific action (which leads to chase, jobs stuck on the floor, and how many customers are due for new tires to call back). Be direct and concrete with the numbers.
Keep it tight and useful. Output ONLY the briefing text.`;
  ...
  try {
    ...
    return res.status(200).json({ briefing, leadStats, ops });
  } catch (e) {
